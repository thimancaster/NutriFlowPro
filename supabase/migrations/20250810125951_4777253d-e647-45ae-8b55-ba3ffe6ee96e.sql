...
-- Fix the generate_meal_plan_with_cultural_rules function
-- Replace problematic SELECT DISTINCT with GROUP BY to resolve PostgreSQL error
...
DECLARE
    ...
    v_food_json JSONB;
    v_total_meal_calories NUMERIC := 0;
    v_total_meal_protein NUMERIC := 0;
    v_total_meal_carbs NUMERIC := 0;
    v_total_meal_fats NUMERIC := 0;
BEGIN
    -- Set date
    ...
    FOR i IN 1..array_length(v_meal_types, 1) LOOP
        ...
        v_foods_array := '[]'::jsonb;
        v_total_meal_calories := 0;
        v_total_meal_protein := 0;
        v_total_meal_carbs := 0;
        v_total_meal_fats := 0;

        -- Get culturally appropriate foods for this meal type (FIXED QUERY)
        SELECT ARRAY_AGG(food_id ORDER BY max_cultural_score DESC, random_order) INTO v_culturally_appropriate_foods
        FROM (
            SELECT 
                f.id as food_id,
                MAX(mcr.cultural_score) as max_cultural_score,
            ...
            GROUP BY f.id
            ORDER BY max_cultural_score DESC, random_order
            LIMIT 3
        ) ordered_foods;
        ...
        IF v_culturally_appropriate_foods IS NOT NULL AND array_length(v_culturally_appropriate_foods, 1) > 0 THEN
            FOR j IN 1..LEAST(array_length(v_culturally_appropriate_foods, 1), 3) LOOP
                ...
                v_food_calories := ROUND(v_food_calories * (v_food_portion / (SELECT portion_size FROM public.foods WHERE id = v_food_id)));
                v_food_protein := ROUND(v_food_protein * (v_food_portion / (SELECT portion_size FROM public.foods WHERE id = v_food_id)), 1);
                v_food_carbs := ROUND(v_food_carbs * (v_food_portion / (SELECT portion_size FROM public.foods WHERE id = v_food_id)), 1);
                v_food_fats := ROUND(v_food_fats * (v_food_portion / (SELECT portion_size FROM public.foods WHERE id = v_food_id)), 1);

                -- Accumulate meal totals
                v_total_meal_calories := v_total_meal_calories + v_food_calories;
                v_total_meal_protein := v_total_meal_protein + v_food_protein;
                v_total_meal_carbs := v_total_meal_carbs + v_food_carbs;
                v_total_meal_fats := v_total_meal_fats + v_food_fats;
                ...
                INSERT INTO public.meal_plan_items (
                    ...
                    v_food_portion, v_food_unit, v_food_calories, v_food_protein, v_food_carbs, v_food_fats,
                    j, NOW(), NOW()
                );
                ...
                v_food_json := jsonb_build_object(
                    ...
                    'quantity', v_food_portion, 'unit', v_food_unit, 'calories', v_food_calories,
                    'protein', v_food_protein, 'carbs', v_food_carbs, 'fats', v_food_fats, 'order_index', j
                );
        ...
        -- Build meal JSON using actual totals
        v_meal_json := jsonb_build_object(
            'id', v_meal_type || '-meal', 'type', v_meal_type, 'name', v_meal_name,
            'foods', v_foods_array, 'total_calories', v_total_meal_calories,
            'total_protein', v_total_meal_protein, 'total_carbs', v_total_meal_carbs, 'total_fats', v_total_meal_fats
        );
