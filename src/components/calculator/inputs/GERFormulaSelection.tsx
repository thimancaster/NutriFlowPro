...
const GERFormulaSelection: React.FC<GERFormulaSelectionProps> = ({
	...
	const recommendedFormula = profile
		? recommendGERFormula(profile, hasBodyFat, age, weight, height)
		: null;
			...
			switch (formula) {
				...
				case "mifflin_st_jeor":
					if (age && (age < 18 || age > 65))
						warnings.push("Menos precisa fora da faixa etária de 18-65 anos");
					break;
				case "owen":
					if (imc < 25)
						warnings.push("Mais indicada para pacientes com sobrepeso/obesidade");
					break;
				case "cunningham":
					if (profile !== "atleta")
						warnings.push("Desenvolvida especificamente para atletas de elite");
					break;
			}
		}
				...
				<CardContent className="space-y-4">
					{recommendedFormula && (
						<Alert className="border-primary/20 bg-primary/5">
							...
							<AlertDescription className="text-primary/90">
								...
								sugerimos a fórmula{" "}
								<strong>{GER_FORMULAS[recommendedFormula].name}</strong>.
					...
					<div>
						<Label htmlFor="ger-formula">
							Equação para Gasto Energético de Repouso (GER){" "}
							{required && <span className="text-red-500">*</span>}
						</Label>
						<Select
						...
						<div className="bg-muted/30 p-3 rounded-md border text-sm">
							...
							<div className="text-muted-foreground space-y-1">
								...
								<div className="mt-2 text-xs">
									Fórmula selecionada:{" "}
									<strong>{GER_FORMULAS[selectedFormula].name}</strong>
								</div>
							</div>
						</div>
					)}
