              ...
              <SelectContent>
                <SelectItem value="eutrofico">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">Harris-Benedict</div>
                  </div>
                </SelectItem>
          ...
          {inputs.objective === 'personalizado' && (
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => updateInputs({ customAdjustment: Number(e.target.value) })}
              />
            </div>
          )}
        </CardContent>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="text-center p-4 bg-background rounded-lg">
                ...
                <p className="text-3xl font-bold text-primary">{results.tmb.value}</p>
                <p className="text-xs text-muted-foreground mt-1">{results.tmb.formula}</p>
              </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-3 bg-background rounded-lg">
                  <p className="text-sm font-medium text-blue-600 mb-1">Proteína</p>
                  <p className="text-xl font-bold">{results.macros.protein.grams}g</p>
                  <p className="text-sm text-muted-foreground">{results.macros.protein.kcal} kcal ({results.macros.protein.percentage}%)</p>
                  {results.proteinPerKg && (
                    <p className="text-xs text-muted-foreground mt-1">{results.proteinPerKg.toFixed(1)} g/kg</p>
                  )}
                </div>

                <div className="p-3 bg-background rounded-lg">
                  <p className="text-sm font-medium text-green-600 mb-1">Carboidratos</p>
                  <p className="text-xl font-bold">{results.macros.carbs.grams}g</p>
                  <p className="text-sm text-muted-foreground">{results.macros.carbs.kcal} kcal ({results.macros.carbs.percentage}%)</p>
                </div>

                <div className="p-3 bg-background rounded-lg">
                  <p className="text-sm font-medium text-orange-600 mb-1">Gordura</p>
                  <p className="text-xl font-bold">{results.macros.fat.grams}g</p>
                  <p className="text-sm text-muted-foreground">{results.macros.fat.kcal} kcal ({results.macros.fat.percentage}%)</p>
                  {results.fatPerKg && (
                    <p className="text-xs text-muted-foreground mt-1">{results.fatPerKg.toFixed(1)} g/kg</p>
                  )}
                </div>
              </div>
            </div>
