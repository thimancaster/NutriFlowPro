...
const PricingPlan: React.FC<PricingPlanProps> = ({
	...
	return (
		<Card
			className={`relative ${
				highlighted
					? "bg-white border-2 border-green-500 shadow-xl shadow-green-500/20 scale-105 z-10"
					: "bg-white border-gray-200"
			} flex flex-col h-full transition-all duration-300 hover:shadow-lg hover:-translate-y-1`}>
			{badge && (
				<div className="absolute -top-4 left-1/2 transform -translate-x-1/2 z-20">
					<Badge className="bg-gradient-to-r from-green-500 to-blue-600 text-white px-6 py-2 font-bold text-sm shadow-lg animate-pulse">
						{badge}
					</Badge>
				</div>
			)}
