...
const EnhancedFoodSearch: React.FC<EnhancedFoodSearchProps> = ({onFoodSelect}) => {
	...
	const allergens = ["gluten", "lactose", "soja", "ovos", "amendoim", "frutos do mar", "nozes"];
	...
	const searchFoods = async () => {
		...
		try {
			...
			const result = await getFoodsWithNutrition(searchFilters);
			let foodsData = result.data;

			// Apply additional filters
			if (filters.maxCalories) {
				foodsData = foodsData.filter(
					(food) => food.calories <= parseFloat(filters.maxCalories)
				);
			}
			if (filters.minProtein) {
				foodsData = foodsData.filter(
					(food) => food.protein >= parseFloat(filters.minProtein)
				);
			}
				...
				<Card className="border-l-4 border-l-primary">
					...
					<CardContent className="space-y-6">
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
							...
							{/* Calories Filter */}
							<div className="space-y-2">
								<label htmlFor="max-calories" className="text-sm font-medium">
									Máx. Calorias (por porção)
								</label>
								<Input
									id="max-calories"
									type="number"
							...
							{/* Protein Filter */}
							<div className="space-y-2">
								<label htmlFor="min-protein" className="text-sm font-medium">
									Mín. Proteína (g)
								</label>
								<Input
									id="min-protein"
						...
						<div className="space-y-3">
							<fieldset>
								<legend className="text-sm font-medium">Livre de Alérgenos</legend>
				...
				{loading ? (
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
						{Array.from({length: 6}, (_, index) => (
							<Card key={`food-search-skeleton-${index}`} className="overflow-hidden">
						...
						<CardContent>
							...
							<p className="text-muted-foreground">
								{searchTerm
									...
									: "Digite algo para buscar alimentos"}
							</p>
						</CardContent>
					</Card>
				) : (
					<>
						<div className="flex items-center justify-between">
							<p className="text-sm text-muted-foreground">
								{foods.length} alimento{foods.length !== 1 ? "s" : ""} encontrado
								{foods.length !== 1 ? "s" : ""}
							</p>
						</div>
											...
											<div className="flex items-center justify-between text-xs text-muted-foreground">
												...
												{food.sustainability_score && (
													<div className="flex items-center">
														...
														Sustentabilidade {food.sustainability_score}
														/10
													</div>
												)}
											</div>
