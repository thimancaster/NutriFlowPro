...
const DashboardRecentPatients: React.FC = () => {
	...
	const {data: recentPatients, isLoading} = useQuery({
		...
		queryFn: async () => {
			...
			console.log(
				"Loaded recent patients:",
				typedPatients.map((p) => ({id: p.id, name: p.name}))
			);
			...
			<CardContent>
				{isLoading ? (
						...
						<table className="w-full">
							<thead>
								<tr className="text-left border-b border-border">
									<th className="pb-2 pt-4 px-4 text-muted-foreground font-medium">
										Nome
									</th>
									<th className="pb-2 pt-4 px-4 text-muted-foreground font-medium">
										Data
							...
							<tbody>
								{recentPatients.map((patient) => (
									<tr
										key={patient.id}
										className="border-b border-border last:border-b-0 group hover:bg-muted/30 transition-colors duration-200">
										...
										<td className="py-3 px-4">
											<span
												className={`px-2 py-1 text-xs rounded-full ${
													getPatientStatus(patient) === "Novo"
														? "bg-nutri-blue-light text-white"
														: getPatientStatus(patient) ===
														  "Em andamento"
														? "bg-nutri-green-light text-white"
					...
					<div className="text-center py-4">
						<p className="text-gray-500">Você ainda não tem pacientes cadastrados</p>
					</div>
				)}
			</CardContent>
			{recentPatients?.length > 0 && (
				<CardFooter className="pt-0">
					<Link to="/patients" className="w-full">
						<Button
							variant="outline"
							className="w-full hover:bg-nutri-blue hover:text-white transition-colors duration-200">
