...
export const useMealPlanCalculations = () => {
  ...
  const calculateItemRefeicao = useCallback(
    (alimento: AlimentoV2, quantidade: number): ItemRefeicao => {
      return {
        ...
        peso_total_g: Number(
          (alimento.peso_referencia_g * quantidade).toFixed(2)
        ),
        kcal_calculado: Number(
          (alimento.kcal_por_referencia * quantidade).toFixed(2)
        ),
        ptn_g_calculado: Number(
          (alimento.ptn_g_por_referencia * quantidade).toFixed(2)
        ),
        cho_g_calculado: Number(
          (alimento.cho_g_por_referencia * quantidade).toFixed(2)
        ),
        lip_g_calculado: Number(
          (alimento.lip_g_por_referencia * quantidade).toFixed(2)
        ),
      };
    },
    []
   ...
   */
  const calculateRefeicaoTotals = useCallback(
    (itens: ItemRefeicao[]): RefeicaoTotals => {
      return itens.reduce(
        (totals, item) => ({
