...
const buildExportOptions = (params: ExportParams, dailyTotals: { kcal: number; ptn: number; cho: number; lip: number }, meals: any[]): MealPlanExportOptions => {
  return {
...
const validateParams = (params: ExportParams, toast: any): boolean => {
...
const prepareData = (params: ExportParams) => {
...
export const useMealPlanExport = () => {
  ...
  const exportToPDF = useCallback(async (params: ExportParams) => {
    ...
    if (!validateParams(params, toast)) return false;

    try {
      const { dailyTotals, meals } = prepareData(params);
      console.log('[PDF_EXPORT] Daily totals calculated:', dailyTotals);

      const exportOptions = buildExportOptions(params, dailyTotals, meals);
  ...
  const printMealPlan = useCallback(async (params: ExportParams) => {
    console.log('[PDF_PRINT] Starting print with params:', {
      patientName: params.patientName,
      refeicoesCount: params.refeicoes?.length,
    });
    ...
    try {
      ...
      console.log('[PDF_PRINT] ✅ Print window opened');
      toast({
      ...
      console.error('[PDF_PRINT] ❌ Error printing:', error);
      toast({
        title: '❌ Erro ao imprimir',
        description: error instanceof Error ? error.message : 'Não foi possível imprimir o plano alimentar.',
        variant: 'destructive',
      });
      return false;
    }
  ...
  return { exportToPDF, printMealPlan };
};
